...
| `winebutik`        | 1    | post-mig. | 2026-08-26 — new store, TS-only (no Python adapter ever existed): winebutik.com.ua Drupal Commerce SSR; the listing sorts purchasable items ahead of a sold-out tail the walk stops at. Live partial run over the first, second and boundary (`?page=45`) pages: 12 + 12 + 6 collected, walk ended on the boundary page as complete/exhausted, no tail page fetched |
