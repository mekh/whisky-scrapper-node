...
- **Migrations added by the knowledge-base work**, after the schema trio:
  ...
  `pnpm reconcile-flavors` already is.
...
| `GET /product/review/producers?status=&page=&perPage=` — producers by review status. The `unverified` page is ranked by `potentialReach` — how many bottlings would resolve to the row if the whole withheld queue went live — because `productCount` is structurally 0 for every withheld row and ranking by it ranks alphabetically. `potentialReach` is null on the other statuses, where `productCount` is a real answer                                                                                                                                                                                                                                                         | `producer:read`                              |
| `GET /product/review/facts?field=type\|country&producer=resolved\|unresolved` — bottlings whose type or country the filters no longer trust, worst-first by how many shops carry them. Each row carries the country's `nameUa`/`icon`, the resolved `producerSlug` (null when nothing resolved) and up to five `stores` links (one per shop, in-stock first) to the listings that produced the fact. **`producer` splits the queue into its two jobs**: roughly nine rows in ten resolve to no producer and are a symptom cured a producer at a time, while the rest have one and are here because their producer's range spans several types, so nothing but a person can settle it | `product:review`                             |
| `GET /product/review/conflicts?attribute=&store=` — unresolved cross-shop contradictions, worst-first by how often each was seen                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                     | `product:review`                             |
...
- **Most producers are still `unverified`** — stored, and invisible to the
  resolver (454 of 796 at 2026-08-29, falling as the queue is worked). That is
  the auto-gate working as designed (a positive peat claim needs independent
  corroboration), not a defect; the cost is recall, never correctness, since an
  unresolved producer loses peat tags rather than gaining wrong ones.
  `/product/review` is where they are promoted — **confirming one applies it to
  the catalogue in the same request** — and
  `docs/kb-research/curation/review-dossier.md` lists the twelve where
  withholding is most visibly wrong.
- **`BRAND_INFO` / `BRAND_KEYS` / `INDEPENDENT_BOTTLERS` / `detectBrandInfo`
  cannot be deleted yet.** 1448 of 4057 bottlings resolve to no producer
  (2026-08-29, down from 1706), and for every one of them `detectBrandInfo` is
  still the only thing supplying a country or type. The gate for removing them
  is that number falling, which happens as producers are promoted and
  `pnpm research-brands` fills the tail.
