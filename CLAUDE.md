...
- `store-product` — one store's **offer** of a bottling: `storeId`,
  ...
  **Out-of-stock offers are never deleted** — the persist pipeline flips
  ...
  `reviewStatus IS DISTINCT FROM 'rejected'` — see "The new-product queue" for
...
### The new-product queue (`product.reviewStatus`) (2026-09-15)

Every bottling a sync creates lands in a work queue, so a person sees it once
before the catalogue keeps it forever. Until this existed nothing ever put a
new row in front of anybody: a truncated name, an age read out of prose, a
gift set recorded as one bottle or a bag of drink ice a shop files under whisky
sat in the reports until somebody happened to trip over it. The screen is the
fourth tab of `/product/review`; the endpoints are `GET /product/review/queue`
and `POST /product/review/status`.

`ProductReviewStatus` (`~enums`) has three values and **null is the fourth
state**: `pending` (a sync made it, nobody has looked), `verified` (somebody
has), `rejected` (somebody ruled it out — usually "not whisky"), and null for
every row that predates the queue. A `varchar(16)` with no Postgres enum and
no CHECK, exactly as `KbStatus` is, which is what let that vocabulary gain
`rejected` later with no migration at all.

**Enrolment is the column's database default, deliberately, and it is the one
thing to understand here.** Three statements insert into `product` —
`FIND_OR_CREATE_SQL`, `createUnmatched`, and the rare fallback inside
`ScrapePersistService.upsertOffer` — and **none of them names this column**.
The default is what enrols their rows, and what a fourth insert path added
later inherits for free. The provenance columns beside it had to be explicit
because their value differs per row; this one is a constant at insert time,
which is what a column default is for. The alternative — the column in three
`INSERT` lists — is three copies of one constant, and the failure mode it
invites is the one the queue exists to remove: a bottling that never reaches
the queue is invisible by construction. `findOrCreateByMatchKeys` therefore
needs no change at all; its per-row `(xmax = 0) AS "isNew"` still only has to
produce the `addedProducts` counter.

**The no-op `ON CONFLICT DO UPDATE` is the other half**, and it is what makes
the queue idempotent between syncs: it assigns `matchKey = EXCLUDED."matchKey"`
and touches nothing else, so a bottling somebody has already reviewed is not
pushed back into the queue when a shop lists it again tomorrow. Verified for
all three states — `pending`, `verified` and a null legacy row — in
`test/integration/product-review-queue.integration.spec.ts`.

**A person's own creation skips the queue.** `ProductService.relink` creates a
bottling through the same `createUnmatched` the sync's fallback uses, so it
stamps `verified` explicitly: every fact on that row is a person's (`manual`
throughout) and the row carries no raw shop name for a cleaned one to be
compared against, which is what a queue entry is read for. Left `pending`, a
reviewer's own correction would add an item to the queue it was shortening.
A relink's _target_, when it already existed, keeps whatever place in the queue
it had — a relink says the listing was on the wrong whisky, not that the
target's facts are wrong.

**An edit of a queued bottling is its review.** `POST /product/update` stamps
`verified` when the row is `pending`, because correcting the parse error is the
act the queue asks for and a second click to confirm it is a click somebody
forgets. Three details are load-bearing:

- the stamp goes on the **merge survivor**, after `mergeTwins`. Before the
  merge it would be discarded in exactly the case the feature exists for — the
  merge keeps the less-settled status, so a `verified` loser folding into a
  `pending` survivor yields `pending`;
- a null row stays null, so editing a bottling that predates the queue from
  the catalogue's own product card does not enrol it — otherwise the decision
  to leave the old catalogue out unwinds one edit at a time;
- a `rejected` row stays rejected. Fixing a name before taking a rejection back
  is fine; taking it back is its own decision with its own button.

**`mergeInto` gives the survivor the least settled of the two statuses**, with
`rejected` outranking everything: `rejected` > `pending` > `verified` > null.
A rejection is a person's decision and must not be undone as a side effect of
somebody editing a name, while a merge does produce a combination of facts
nobody has looked at in that combination, so a queue entry survives being
absorbed by a verified twin. Two null rows stay null. The chain is written with
`IS NOT DISTINCT FROM`, not `=`: a `NULL = 'rejected'` comparison yields NULL
and the `CASE` falls through to the next `WHEN`, so `=` happens to be right
today — and relying on null-as-no-match inside a `CASE` is the kind of
subtlety that breaks silently under the next edit. `deleteIfUnreferenced` is
unchanged: the queue is a column, not a reference, so an emptied shell is
deleted along with its place in it.

**`rejected` is a read filter, not a deletion.** The row stays, its offers keep
being upserted and its price snapshots keep being written, so the verdict costs
no data and is reversible at any time. What hides it:
...
And what deliberately does **not**, each for a reason:
...
| `ProductService.search` → `GET /product/search` | the relink picker must reach it in order to move a listing **off** it, the same exemption the blacklist picker already has                                                                      |
...
| `/dashboard/*`                                  | a historical record of what shops listed on a given day; a curation decision made later must not rewrite it. A `by=reviewStatus` breakdown would be the right way to surface this, not a filter |

**A rejected bottling absorbs its own future listings, and that is the
"the next sync does not resurrect it" guarantee.** It stays in key and
identity resolution (`KNOWN_KEYS_SQL`, `IDENTITY_BATCH_SQL`), so a listing
spelled the same way lands back on it rather than minting a fresh `pending`
row for somebody to reject again. The cost to know about: a rejection made in
error makes a later legitimate listing silently invisible **and** puts it in no
queue, which is why the `rejected` bucket of the tab is readable rather than
`pending` alone.

**The `legacy` counter is served rather than derived.** `GET
/product/review/summary` carries `products: {pending, verified, rejected,
legacy}`, the four sum to the whole table, and `legacy` is the share of the
catalogue nobody has ever looked at — the number a retro-enqueue decision is
made on. That enqueue is one statement, and it is idempotent because null is
what "never queued" means:

```sql
UPDATE product SET "reviewStatus" = 'pending'
WHERE "reviewStatus" IS NULL AND "createdAt" >= now() - interval '30 days';
```

**One mutation writes the column, not three.** `verify`, `reject` and
"put it back" differ only in the value — un-rejecting _is_ verifying with a
different one — and three routes writing one column would be three places to
forget the cache bump. It is **bulk** from the start (`@ArrayMaxSize(200)`,
the screen's own page size): a pass over a night's arrivals is twenty
decisions, and twenty requests against the 3/s limiter would earn a 429 doing
nothing unusual. The bulk shape is also what makes the unconditional cache
bump cheap — see "Catalogue cache".

`ProductRepository.applyReviewStatus` is the **only** writer of the column
outside the migration and the insert default, and its `onlyWhenPending` flag is
what the edit path passes. Everything else — the relink's create, the three
buttons — goes through it too, so the rule about what a decision does lives in
one place.
...
Then `product-review-status` (2026-09-15), the two columns of the new-product
...
| `ProductReviewService.setStatus`   | `product:review` (whatever the verdict — see "The new-product queue")                          |
...
| `GET /product/review/summary` — counters for the curation screen's tabs. `untrustedFacts` is the **distinct** number of bottlings with either fact untrusted; `untrustedTypes`/`untrustedCountries` are the per-field counts and must never be summed (892 bottlings carry both); `untrustedFactsUnresolved` is how much of that queue resolves to no producer at all — the half that is a symptom rather than work. `products` carries the new-product queue's four counters, whose sum is the whole `product` table — `legacy` is the share that predates the queue and is deliberately not in it                                                                                                                                                                                                                                                                                                                                     | `product:review`                             |
...
| `GET /product/review/facts?field=type\|country&producer=resolved\|unresolved&name=` — bottlings whose type or country the filters no longer trust, worst-first by how many shops carry them. Each row carries the country's `nameUa`/`icon`, the resolved `producerSlug` (null when nothing resolved), the shop's own `brandOrig` — the key `POST /producer/:id/alias` is minted from, which is what makes the unresolved half actionable from this screen — and up to five `stores` links (one per shop, in-stock first) to the listings that produced the fact. **`producer` splits the queue into its two jobs**: roughly nine rows in ten resolve to no producer and are a symptom cured a producer at a time, while the rest have one and are here because their producer's range spans several types, so nothing but a person can settle it. `name` is a case-insensitive substring of the canonical name or any store's raw name | `product:review`                             |
...
| `GET /product/review/queue?reviewStatus=&name=&store=&page=&perPage=` — one bucket of the new-product queue, **newest first** (a deliberate departure from the facts tab's ranking by reach: the question is what last night's sync brought in, and a day-old bottling's reach is not yet known). `reviewStatus` defaults to `pending`; the `rejected` bucket is readable because a rejection made by mistake is only reversible if the row can still be found. A row carries `name` beside `nameOrig` — the comparison the screen exists for — plus `matchKey` (nowhere else in the API, and the only cheap moment to notice a wrong one), the specs with their `*Source`, `brandOrig`, the flavour tags, up to five shop links and `createdAt`                                                                                                                                                                                        | `product:review`                             |
| `POST /product/review/status` `{productIds, reviewStatus}` → `200 {updated, products}` — record a verdict on up to 200 bottlings. One route for all three transitions, since they differ only in the value written and un-rejecting _is_ verifying; **bulk** because a pass over a night's arrivals would otherwise earn a 429 from the per-caller limiter. `rejected` removes the bottling from every report, from `/meta` countries, from a shop's `productCount` and from the push digest, while `/report/history`, `/product/search` and `/collection` still answer for it — see "The new-product queue" for why each                                                                                                                                                                                                                                                                                                               | `product:review`                             |
...
| `POST /producer` — create a producer **and apply it**, answering `{producer, applied}` like the patch does. The name is always added as an alias alongside whatever `aliases` the body lists: a producer nothing resolves to changes nothing. Defaults to `status: verified` — a person typed the row, which is what that status means. A taken slug is `409`; a `parentId`/`bottlerId` naming no producer is `400`, not a foreign-key `500`                                                                                                                                                                                                                                                                                                                                                                                                                                                                                            | `producer:create`                            |
| `POST /producer/:id/alias` `{brand, scope?, note?}` — point one shop spelling at a producer **and apply it**, answering the reconcile summary. This is what the facts queue's «Прив'язати» writes, and an alias rather than a per-bottling link on purpose: the resolver reaches a producer only through `producer_alias`, so one alias fixes every bottling carrying that spelling and the country and type then flow from the knowledge base. `brand` is normalized by `KbKeyUtils.key`, so a raw shop string may be pasted; a spelling another producer already claims is `409` naming it, while re-adding this producer's own is a no-op                                                                                                                                                                                                                                                                                            | `producer:update`                            |
