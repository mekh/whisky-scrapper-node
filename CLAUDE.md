...
- **`LOG_JSON=true` is pinned in `docker-compose.yaml` and the pipeline depends on it.** Production used to run `pino-pretty` single-line **with ANSI colour**, which is the worst possible input for a collector. With JSON, `level`, `time`, `context`, `reqId` and an `err` object's every field arrive as fields LogsQL filters on. The collector parses optimistically and keeps the raw line when the parse fails, so flipping it back degrades rather than breaks — but it degrades every replica at once.
