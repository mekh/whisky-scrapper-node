...
| A live VictoriaLogs v1.52.0, fed through the sink's own endpoint and `VL-*` headers | 6 rows in, 0 dropped; **5 streams** of exactly the intended shape; all three dashboard queries return; a synthetic request id, a client IP, a status range and a free-text substring all still filter as ordinary fields; a malformed query is refused `400` rather than answering empty |

Then, against the live deployment: the whole chain confirmed end to end —
`_time` carrying pino's own millisecond ISO stamp rather than the ingest time,
`level` a string name rather than a number or an ANSI-wrapped one. Two things
that verification found and nothing local could have: the sync source's glob
was reading `db_backup.log`, and `context` arrived empty on every record
because the logger folds it into the message. Both fixed, both pinned by
`tests:`.
