...
- `product` — the **bottling**, independent of who sells it: `matchKey?`
  ...
  `lastLlmFlavorAt?`, `reviewStatus?`/`reviewedAt?` (see "The new-product
  queue" below), plus one `<field>Source` column per fact (see
...
- `store-product` — one store's **offer** of a bottling: `storeId`,
  ...
  **A bottling's identity for curation is its name, volume and age (2026-09-06)**, compared with `identityOf` in `ProductRepository` — case-folded, everything but letters and digits removed, so `Ballantine's` equals `Ballantines` and `Double Wood` equals `DoubleWood` while word order and every real letter still count; strength is left out for the reason the key leaves it out. This is deliberately narrower than the match key: the key is what a _listing_ is matched by, the identity is what two _catalogue rows_ are compared by, and it must never fold two whiskies a person spelled differently on purpose. Three things now run on it. **An edit merges** (`ProductService.update`): after every `POST /product/update` the bottling is compared against the catalogue and every row that shares its identity is folded into the most-listed one — the edited row moves onto it, and the person's `manual` values win the merge — so "rename `Arran Amarone Casc` to `Arran Amarone Cask`" is a one-step correction, and an old duplicate is folded away the first time either row is touched. **A relink moves one offer** (`POST /product/relink`, `ProductService.relink`): the correction for a listing the matcher filed under the wrong whisky, which an edit cannot express because every field there belongs to the bottling; the target is an existing bottling by id, or a description the server looks up by identity and creates only when nothing matches (every fact stamped `manual`, no key), and the bottling the offer left is deleted when nothing refers to it any more. **The persist path resolves by identity before it creates** (`findOrCreateByMatchKeys`): a key the catalogue holds — live, or retired into `product_match_alias` — resolves outright; a key nobody holds is then matched against the rows sharing the cleaned name, volume and age; only what survives both is inserted. That closes the brand-token door (`William Peel` was signed `peelwilliam` by one shop and `peelwilliamwilliampeel` by another).
  **`product_match_alias` is where a merged row's key goes.** A key is frozen and never re-derived, so the vanishing row's key is exactly what the next listing spelled the same way derives again — and before the table existed, the duplicate a merge had just folded away was recreated by the next sync. `ProductRepository.mergeInto` moves everything the vanishing row owned (facts by the trust rules of the canonical write, flavor links keeping the better-trusted source with a curated set winning wholesale, offers with their price history, the conflict log, every user's favorite, blacklist and collection entry guarded per user, purchases first), retires its key into the alias table — or hands it to a keyless survivor outright — and **deletes the row**. Deleting is new: the earlier regroups kept the emptied rows, which is what left 51 orphans answering searches and identity lookups. `findByMatchKeys` resolves aliases too, so the enrichment gate sees the facts of the row a listing will actually land on.
  ...
  **Out-of-stock offers are never deleted** — the persist pipeline flips
  ...
  Those same list reads carry the other global predicate,
  `reviewStatus IS DISTINCT FROM 'rejected'` — see "The curation queue" for
  what it hides and, more importantly, for the five reads that deliberately do
  **not** carry it.
...
- `producer_alias` — every spelling that resolves to a producer, normalized by
  ...
  (`brand`|`name`|`lead`|`any`) plus a five-character floor keeps a short
  alias from mis-firing as a substring: the catalogue holds
  `Elements of Islay`, `M&H Elements` and `Glenmorangie Elementa`.
  **`lead` (2026-09-17) is the exception, and the exemption is what it is
  for**: it matches only at the **start** of a normalized name and skips the
  floor, because shops write names brand-first. `hyde` is four letters, so
  brand scope was its only home and a shop that states no brand left eleven
  bottlings unreachable; at the start of a name the same word is the brand,
  while `blue` anywhere in a name takes every `Johnnie Walker Blue Label` and
  at the start takes none of them. Which scope an alias gets is decided per
  alias, by a person reading the two numbers of the impact preview, and never
  by a global rule. `KbAliasUtils.reachesName` is the one statement of what a
  spelling can reach, so the matcher and the curation screen's
  `alias-unreachable` detector cannot disagree.
...
- `product_fact_conflict` — one row per (product, store, attribute) recording a
  ...
  **A re-sighting no longer clears `resolvedAt`** (2026-09-17): every
  unacknowledged contradiction now queues its bottling, so a claim that keeps
  arriving would queue it again every night and an acknowledgement would mean
  nothing. The claimed and stored values are still overwritten, so an
  acknowledged row states today's disagreement. See "The curation queue".
...
**The bottler slot takes the resolved producer's recorded owner first**
(2026-09-18), and only then a bottler an alias found in the name or the brand.
`KbIndex` therefore carries a by-id map of every live producer, loaded beside
the alias index: a range's owner is named by no alias the bottling carries, and
a bottler with no alias of its own — `Chapter 7` — is absent from that index
entirely. This is what `KbProducerFacts.bottlerId` always promised and
`bottlerOf` never did, so `Big Peat` reported Douglas Laing on the five
bottlings that spell the company out and on none of the others, and
`Prologue`'s three bottlings reported nobody at all; the pass filled the slot
on **85** stored bottlings. The order is the honest one for this slot rather
than the usual "the bottling's own name is more specific": the owner of a range
_is_ its bottler, while a second bottler in the title is more often the
retailer that commissioned the cask (`Big Peat Whisky Exchange Edition` was
filled by Douglas Laing). Measured before the change, the two answers disagreed
on **0** of the 103 bottlings that hold either, so the ordering is a decision
about the next such row rather than a repair. `kb-golden.tsv` records the one
expectation it moved.
...
### The producers section (2026-09-17)

Full CRUD over the four kinds of maker, as a catalogue a person browses rather
than the work queue `/product/review` serves: `GET /producer` (filtered,
sorted, paginated), `POST /producer`, `GET`/`PATCH /producer/:id`, and the two
alias routes. `ProducerService` (`domain/product`) owns the writes;
`ProducerController` gained the routes beside the ones it already had.

- **There is no `DELETE`, deliberately.** `rejected` already is the verdict
  "this is not a whisky producer", and unlike a delete it is auditable,
  reversible, and stops `pnpm research-brands` paying to look the name up
  again. A row is also pointed at by products, by its children's `parentId`
  and by everything bottled under it, so a delete would silently orphan facts
  a person curated.
- **Every write applies itself**, the rule `PATCH /producer/:id` already
  followed: a stored claim changes nothing a filter reads until the catalogue
  is re-resolved, so the create and both alias routes run
  `KbReconcileService` in the same request and answer what it wrote.
- **A create is one transaction** (2026-09-19). `POST /producer` takes the
  spellings _and_ the rules, and `CoreProducerService.createProducer` writes
  the row, its `producer_alias` rows and its `flavor_rule` rows under one
  `@Transactional()`, so a rule that collides takes the producer back with it
  rather than leaving a maker nobody asked for. Everything is validated first
  — `ProducerRuleFactory` normalizes the patterns and resolves the tags before
  the write — so the transaction can only fail on the database's own
  constraints, and the reconcile pass runs _after_ the commit, where a
  catalogue-wide rewrite does not sit inside the lock.
- **`ProducerRuleFactory` is the one place a rule is validated**, extracted
  from `ProducerReviewService` when the create started needing it: the XOR
  between a peat band and a tag claim, the `KbKeyUtils.key` pattern and the
  refusal of an unknown flavour. It answers a draft without a `producerId`,
  which is exactly what a create has at that point.
- **`sort` absent is the third sort state**, not a missing parameter — it
  means the listing's own order, most bottlings first. The column expressions
  live in a closed map (`LIST_SORT_SQL`), since the value reaches an
  `ORDER BY`; `peatProfile` sorts by band rather than alphabetically, through
  `array_position`.
- **`ProducerReviewRow` is the one projection**, shared by the review queue,
  the listing and the card (`REVIEW_ROW_SQL`). It gained `createdAt` and the
  two links' ids and names; a second projection is what let the review listing
  and the detail read drift apart once already.
- **The owner field suggests, it does not constrain.** `GET /producer/owner`
  reads `producer.owner` back as a distinct list, because the column has no
  lookup table and a company nobody has recorded is the ordinary case. What
  the list buys is the one thing a free column cannot: `Campari Group`,
  `Campari group` and `Campari  Group` stop becoming three owners of one
  distillery.
- **An alias is never stolen.** `producer_alias.key` is unique across every
  producer precisely so one spelling cannot resolve two ways, so a key another
  producer holds is a `409` naming it rather than a move. A create whose extra
  spellings collide skips them instead of failing: the row is already written,
  and refusing it over one duplicate would leave the person with nothing.
- **A taken slug names its holder, and that closes a dead end** (2026-09-19).
  The picker hides `rejected` rows on purpose and the slug index does not know
  about status, so a maker ruled not-whisky is invisible _and_ owns its name
  forever: the reviewer searched `Vulson`, found nothing, was offered «створити»
  and got a bare `409`. The refusal now carries `{id, slug, name, status, note}`
  — the stored verdict included, since deciding whether that verdict was right
  is what the reviewer is now being asked to do. The route's behaviour is
  otherwise unchanged; what changed is that the answer is actionable.
...
### The curation queue (`/product/review`) (2026-09-17)

One queue of bottlings that need a person, each row carrying **why** it is in
it. It replaced the four-tab screen wholesale — none of its tabs, predicates,
endpoints or widgets survive — because the owner works this screen himself and
ruled every one of them broken, misleading or beside the point. The design,
the census it was measured on and every decision are in
[`docs/REVIEW-REDESIGN.md`](docs/REVIEW-REDESIGN.md).

**Membership is the union of the detectors**: a bottling is queued when it is
neither `verified` nor `rejected` **and** at least one reason fires. `new` is
itself a reason, so every bottling a sync creates appears even when nothing
else is wrong, while a legacy (`null`) bottling appears only when a detector
finds something — the ~3 500 clean legacy rows never enter.

**Every predicate lives in one SQL fragment**, `REVIEW_ISSUES_SQL`
(`core/_common/review-issues.sql.ts`, shared by `core/product` and
`core/producer`). That fragment produces the row's `issues text[]`, the
`issue=` filter is an overlap test against the same array, and the summary's
per-code counts are a tally of it — so the chip a person filters by and the
chip on the row cannot disagree. Measured on the dump of 2026-09-17, over
3 213 stocked bottlings:

| Code                | Severity | Fires when                                                                                   |       Rows |
| ------------------- | -------- | -------------------------------------------------------------------------------------------- | ---------: |
| `producer-rejected` | critical | no producer and the what-if resolution reaches a producer somebody ruled not whisky          |         17 |
| `no-producer`       | error    | `producerId IS NULL AND bottlerId IS NULL`                                                   |        114 |
| `producer-withheld` | error    | the what-if resolution reaches a producer the knowledge base still withholds                 |          0 |
| `cyrillic-name`     | error    | `name !~ '[A-Za-z]'` — a transliteration no alias, identity or key can ever join to its twin |          7 |
| `duplicate`         | error    | another bottling shares the folded name, volume and age                                      |          2 |
| `missing-<fact>`    | error    | `abv` / `volumeMl` / `countryId` / `typeId` null                                             | 23/5/14/19 |
| `type-vs-name`      | warn     | a type word in the name or a raw name contradicts the stored type                            |         61 |
| `country-vs-name`   | warn     | a Scotch region word in the name, country elsewhere                                          |          2 |
| `abv-range`         | warn     | strength outside 35–70 %                                                                     |         28 |
| `name-leftover`     | warn     | packaging still in the canonical name                                                        |          2 |
| `age-in-raw`        | warn     | a shop's raw name states an age the bottling does not carry                                  |         19 |
| `untrusted-fact`    | warn     | `typeSource`/`countrySource` is `llm` or `legacy`                                            |          6 |
| `conflict`          | warn     | an **unacknowledged** `product_fact_conflict` row                                            |          0 |
| `new`               | info     | `reviewStatus = 'pending'`                                                                   |        125 |
| `age-single-store`  | info     | `ageSource = store` and one shop lists it                                                    |          4 |

Six things about that table are decisions rather than mechanics:

- **Two detectors cannot be written in SQL and are handed in.**
  `producer-rejected` and `producer-withheld` ask what a bottling _would_
  resolve to if the inert rows were in the index, and answering that in SQL
  would be a second implementation of alias matching — the exact defect class
  the knowledge base exists to remove. So `ProducerReachService.inertHits()`
  runs the **real** resolver over an index the `rejected` and `unverified`
  rows are added to, once per request (~130 ms over the catalogue), and hands
  the query two id arrays. The producer's _name_ is filled in afterwards, in
  TypeScript, because "resolves to Yakusun, ruled not whisky" is the sentence
  and a bare id is not one.
- **The type vocabulary is a keyword table, not a regex.**
  `REVIEW_TYPE_PATTERNS` (`~constants/review.constants.ts`) is read by the SQL
  — which is generated from it — and by the TypeScript that explains the chip,
  because POSIX and JavaScript regular expressions differ in exactly the
  places this vocabulary needs: word boundaries around Cyrillic, lookahead
  syntax, case folding. Its **exclusions** are the load-bearing part, and they
  are the same lesson `ProductNameUtils` already records: `bourbon` is a
  maturation word in this catalogue far more often than a category word
  (`1st fill bourbon hogshead`, `bourbon & sherry casks`, `ex bourbon`), so
  the bare word claims nothing. The two large buckets that survive — 19
  rye-named bottlings stored as `bourbon`, 15 single-malt-named stored as
  `blend` — are all genuine.
- **A detector that knows the answer says so in `detail`.** `type-vs-name`
  carries the type the name states and `country-vs-name` carries
  `REVIEW_SCOTLAND_CODE` — it fires on Scotland's regions and on nothing
  else, so the country it means is a constant. Both are what the screen's
  one-click fix is built from, and stating them here is what keeps the
  vocabulary on this side: the alternative is the client carrying its own
  copy of «Islay means Scotland». Pinned by
  `review-issues.integration.spec.ts`.
- **`duplicate` is the exact identity twin only.** The near-twin (same maker,
  same folded name and volume, exactly one of the pair stating an age) fires
  on **302** stocked bottlings against the exact rule's two, because the age
  is stripped from the canonical name: a NAS `Glenfiddich` and a twelve-year
  old `Glenfiddich` are one folded name and are genuinely two whiskies. It is
  a real signal and a useless queue, so it lives in the per-bottling
  suggestions instead, where a person is already looking at the row.
- **A contradiction now queues its bottling, and an acknowledgement sticks.**
  `logFactConflicts` no longer clears `resolvedAt` on a re-sighting (it bumps
  `seenCount`/`lastSeenAt` only) and the `fact-conflict-acknowledge` migration
  stamped the 790 rows open at release, which the owner had already worked
  through. Without the first half the second would have been worth one night;
  without the second, 391 bottlings would have entered the queue on deploy
  carrying a decision already made. The rows stay and the
  `includeAcknowledged` flag brings them all back — a contradiction is
  evidence whether or not it queues anything. **This reverses the rule
  documented here before 2026-09-17.**
- **Severity, then newest, is the default order**, and the severity of each
  code comes from `REVIEW_ISSUE_SEVERITY` — the same map the client colours
  its chips by, so "the queue's worst row" and "the reddest chip" are one
  statement.
- **`age-in-raw` deliberately ignores "aged from N years" prose.** `витримка
  4 роки` and `від 3 років` state a minimum, not an age statement; counting
  them took the queue from 19 rows to 62, most of them nothing to do.

**Nothing automatic re-opens a `verified` bottling** — the owner's decision.
Facts a person wrote are `manual`, so no sync and no knowledge-base pass can
move them; facts left alone keep their source and may still improve, and a
change to one lands silently. The only way back into the queue is a person
pressing "Повернути в чергу".

`ProductReviewStatus` (`~enums`) still has its three values and null is still
the fourth state (a `varchar(16)`, no Postgres enum, no CHECK — which is what
let `KbStatus` gain `rejected` later with no migration). **Enrolment is still
the column's database default**: the three statements that insert into
`product` name no `reviewStatus`, so a fourth insert path added later enrols
its rows for free, and the no-op `ON CONFLICT DO UPDATE` is what keeps the
queue idempotent between syncs.

**`rejected` is a read filter, not a deletion**, and the five reads that hide
it and the five that deliberately do not are unchanged — see the two tables
below.

| Read                                                               | Why it hides a rejected bottling                                                                                                                                                                                  |
| ------------------------------------------------------------------ | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `findCurrentRows` → all five `/report/*` kinds                     | one `AND`, since `buildGroups` is the single selector                                                                                                                                                             |
| `distinctCountries` → `/meta` countries                            | a "not whisky" row must not contribute a country to the whisky filter                                                                                                                                             |
| `countByStore` → `/store/:slug` `productCount`, `GET /store` items | a shop's listing count should not count non-whisky                                                                                                                                                                |
| `claimDrops` → the push digest                                     | **the one that must not be forgotten**: every other read makes something invisible, this one _sends a notification_, and a push announcing a discount on something a person ruled not-whisky cannot be taken back |
| `findFlavorCandidates` → `pnpm enrich-flavors`                     | not paying a model to recall the flavour profile of something that is not whisky                                                                                                                                  |

| Read                                            | Why not                                                                                                                                |
| ----------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------- |
| `findCurrentRowById` → `GET /report/history`    | the verdict is reversible, so the product card has to keep answering for it; filtering here makes un-rejecting unreachable from the UI |
| `resolveIdByTerm` → `/report/history?term=`     | the same — a rejected bottling has to stay findable to stay reversible                                                                 |
| `ProductService.search` → `GET /product/search` | the relink picker must reach it in order to move a listing **off** it                                                                  |
| `findCurrentRowsByProductIds` → `/collection`   | a bottle already on somebody's shelf keeps its price beside it whatever the catalogue has since decided                                |
| `/dashboard/*`                                  | a historical record of what shops listed on a given day; a curation decision made later must not rewrite it                            |

**A rejected bottling still absorbs its own future listings** — it stays in
key and identity resolution, so a listing spelled the same way lands back on
it rather than minting a fresh `pending` row for somebody to reject again.

#### One commit per decision

`POST /product/review/commit` records everything a person decided about one
bottling and stamps the verdict, in one transaction: the fact patch (every
written field → `manual`), the `confirm[]` list (→ `manual` without changing a
value), the producer link, the acknowledgement of every open contradiction,
and the verdict itself. The order is load-bearing:

- **the reach is measured before the write**, so the toast repeats exactly
  what the person agreed to;
- **the facts are written next**, because the identity merge they can trigger
  decides which row the verdict lands on — the stamp goes on the survivor;
- **the knowledge-base pass runs after the transaction**, since an alias
  changes what the whole catalogue resolves to and nothing inside one
  bottling's transaction should carry that.

**`producerSource = 'manual'` is a new write path.** `SET_PRODUCERS_SQL` has
always respected it and nothing could write one, so a bottling no spelling
reaches had no way to be given a maker at all except by inventing an alias for
it. `CoreProductService.setProducerManual` is the escape hatch, and an
integration test proves a pinned link survives a full reconcile.

**A pin refuses a bottler in the producer slot**, and so does a bulk producer
write: `POST /product/review/commit` with `producer.mode = 'pin'` and
`POST /product/review/bulk` both answer `400` when the id names a
`bottler`-kind row (the same request also answers `400` when nothing has that
id — the bulk route's own existence check, widened). The resolver has always
refused that slot to a bottler, on the grounds that the facts would then be
read off a company that owns no still, and a pin was the one path that could
write one anyway. The picker still offers all four kinds deliberately: an
**alias** to a bottler is how an independent bottling resolves at all. To
record a bottler with no distillery behind it, give the range row its
`bottlerId` instead — the resolver fills the slot from there.

#### Reach before commit

Every action that touches other bottlings says how many, which ones and what
changes, with a link to each — the owner's rule. `ReviewPreviewService` builds
a hypothetical alias index, runs `KbApplyService.plan` over the whole
catalogue and diffs the plan against the stored rows. Two numbers, defined
exactly because the owner asked what "changes producer" means for a bottling
that had none:

- **`frees`** — bottlings that resolve to nothing today and would resolve to
  something. A bottling gaining its first producer belongs here and nowhere
  else.
- **`steals`** — bottlings that resolve to a **different** producer today and
  would be re-pointed. The danger number.

For an alias action the same pair is answered **per scope**, so the narrowest
option that fixes the queue is visible before it is chosen. A `pin` is
answered without a pass at all: it writes `manual`, so it reaches exactly one
bottling by construction.

**An alias write rescopes a spelling its own producer already holds.** That is
the whole `Hyde` case: the key exists, brand-scoped, and the person is
choosing where it may be matched; a no-op there would leave them pressing a
button that does nothing. A key **another** producer holds is still a `409`,
never a move.
...
Then `product-review-status` (2026-09-15), the two columns of the curation
queue — `reviewStatus varchar(16)` and `reviewedAt` — plus
`product_review_status_idx`. **It is two `ALTER`s, and that is the mechanism
rather than a stylistic split**: `ADD COLUMN ... DEFAULT 'pending'` in one step
gives the default to the rows already there too, so all 3 839 bottlings would
have landed in the queue on deploy — enrolling the existing catalogue is a
separate decision with its own window. Adding the column without a default
leaves them null; setting the default afterwards reaches only inserts that omit
...
Then `fact-conflict-acknowledge` (2026-09-17), one `UPDATE` stamping every
open cross-shop contradiction acknowledged. It pairs with the other half of
the same reversal, in `logFactConflicts`: an unacknowledged contradiction now
queues its bottling, and a re-sighting no longer clears `resolvedAt`, so
without the migration 391 bottlings would have entered the curation queue on
deploy carrying a decision the owner had already made — and without the code
change the migration would have been worth one night. Nothing is lost: the
rows stay, and the screen's «переглянуті розбіжності» checkbox brings the
whole set back. `down()` is a documented no-op, since the rows it stamped are
indistinguishable from the ones a person acknowledged afterwards.
...
- **Adapters** — every store the project scrapes now has one: `zakaz/` (one
  ...
  **The shop states the maker nowhere but inside the name** — `(Країна, ТМ
  Brand)` at the end of every listing it knows one for, which the name cleaner
  strips, so 60 of the 67 unresolved bottlings it contributed carried a brand
  the catalogue never read. `BrandHintUtils.fromRawName` (`~utils`, the one
  place that regex lives, in both the Cyrillic `ТМ` and the Latin `TM`
  spellings) lifts it into `snap.brand`, so it reaches `brandOrig` and
  whole-string brand matching — which has no length floor and no scope
  problem, and therefore resolves a four-letter maker with no alias edit at
  all.
...
- **TypeORM `.query` gotcha**: `INSERT ... RETURNING` yields a flat rows array,
  ...
  builder's `.execute().affected` for update/delete counts — or, when the
  statement is raw SQL and its rows are wanted,
  **`BaseRepository.updateReturning`**, which unwraps that envelope in one
  place. Reading the raw result's `length` gives **2** for every update that
  matched anything, which is a count that looks plausible and is never right:
  it shipped once as «записано рядків: 2» on a bulk verdict that wrote
  thirteen, and five call sites of the curation work had it. Pinned by
  `review-commit.integration.spec.ts`.
...
**A refusal answers its bare message as `text/plain`, unless it marks its data
exposed.** `ErrorBase.data` is a debugging aid — it names ids nobody asked for
— so the filter has always sent the message alone. An error constructed with
`expose: true` answers `{message, data}` as JSON instead, for the one case
where the client has to _act_ on the refusal rather than print it: `POST
/producer` naming the producer that already holds the slug (see "The producers
section"). Opt-in per error, so every other 4xx keeps the shape `../web`'s
fetch mutator has always read.
...
- **The ladder states its own standing, and `AuthThrottleInterceptor` is what writes it** (2026-09-15): `X-Login-Attempts` (the run length, 5), `X-Login-Attempts-Remaining` (failures left before the next penalty) and, when a penalty is in force, `X-Login-Retry-After-Ms`. They go on the `401` a wrong password earns as much as on the `429`, which is the point — `../web` draws a count under the login form from the second failure on, so a person sees the door closing rather than finding it shut. Four things worth knowing:
  - **The interceptor writes them because only it can.** The count comes from the write that follows the handler (`registerFailure` now answers with the standing) and the deadline from the check that precedes it; a guard sees neither, and `ExceptionFilter` sees only the error. The refusal carries its standing in `TooManyRequestsError.data` for the same trip.
  - **They are deliberately not the `X-RateLimit-*` family.** Both limits can refuse one request, for different reasons and with different waits, so one set of headers could not describe both.
  - **`X-Login-Retry-After-Ms` is a penalty, `Retry-After` is any wait.** A refusal for the one-per-second spacing states the latter and no penalty at all — a second of spacing is not a block, and a client that drew a countdown for it would flash one on every double submit. The scripts therefore answer the block's remainder separately from the refusal's wait.
  - **Nothing is written when the cache could not say.** The ladder fails open, and a client told "four attempts left" by a process that does not know would be worse served than one told nothing. `remaining` also reads full while a penalty is in force — the run is granted afresh when the wait ends — so a blocked answer is read by its deadline, never by its count.
  - The scripts hand back a raw failure count and the service applies `LOGIN_ATTEMPTS_PER_STAGE` to it, so the run length still lives in `~constants` alone. The standing is pinned against a live Valkey in the integration spec, since that arithmetic is in the Lua.
...
| `ProductReviewService.setStatus`   | `product:review` (whatever the verdict — see "The curation queue")                             |
| `ReviewCommitService.commit`       | `review:commit`, and `review:merge` / `review:bulk` for the other two writes                   |
...
**A redaction miss now costs more than it did.** Until 2026-09-16 a leaked value scrolled past in `docker logs` and was gone with the next restart; it is now shipped to VictoriaLogs, kept for thirty days and **searchable by anyone with the Grafana login**. The rule is unchanged, the consequence of breaking it is not.
...
### Where the logs go (2026-09-16)

Everything this process writes to stdout is collected by **Vector** and stored in **VictoriaLogs** for thirty days, searchable from the same Grafana that draws the metrics. Not Loki: the argument, the operator evidence and the measurements are in [`docs/LOGS-PLAN.md`](docs/LOGS-PLAN.md) §2, and the operator procedures are in [`infra/README.md`](infra/README.md). Three sources — this application's container stdout, the nginx access and error logs, and the per-run scrape files under `./log`, which are cross-run searchable for the first time (`GET /store/:slug/sync-log/:id/file` serves one run at a time and always will).

Four things about it are load-bearing for anything written later:

- **`LOG_JSON=true` is pinned in `docker-compose.yaml` and the pipeline depends on it.** Production used to run `pino-pretty` single-line **with ANSI colour**, which is the worst possible input for a collector. With JSON, `level` (a string name, not pino's number), `time` and an `err` object's every field arrive as fields LogsQL filters on. **`context` is the exception and is worth knowing about**: `LoggerModule.logMethod` composes `msg` as `[context, message].join(': ')` and a real `context` field exists only on the TypeORM child logger, so every ordinary record carries its context inside the message text. `infra/vector/vector.yaml` lifts it back out into a field — non-destructively, leaving `msg` whole — which is what makes `context:="ScrapeService"` a usable filter. Verified against the first production boot, where it had come through empty. The collector parses optimistically and keeps the raw line when the parse fails, so flipping it back degrades rather than breaks — but it degrades every replica at once.
- **A fact stated once: the sync log files carry their date only in the filename**, and `SyncFileLogTime` writes both the filename stamp and the per-line clock from **the process's own timezone** (`getHours()`, `getDate()`), which is UTC because no `TZ` is set on the `be` service. `infra/vector/vector.yaml` pins `timezone: UTC` as the other half of that agreement and rebuilds each line's timestamp by joining the two. **Setting `TZ` on `be` means editing that file in the same commit**, or every scrape-log line lands at the wrong hour — the same paired-vocabulary hazard as the age reader and the name stripper, and it fails just as silently.
- **`VL-Stream-Fields` is a cardinality decision, not a convenience.** It holds three fields drawn from closed sets — `source`, `container_name`, `store` — for tens of streams in total. Everything else, request ids and client addresses included, stays an ordinary field: fully searchable in LogsQL, and it does not multiply streams. Adding anything per-request there recreates the exact failure Loki was rejected for. `infra/README.md` has the query that checks it.
- **A new log call site is still checked against `LOG_REDACT_PATHS`**, and the redaction section above says why that now matters more.

What is deliberately **not** collected: this stack's own monitoring containers (a collector that ships its own output is a feedback loop). And there is no alerting on log content yet — the rule worth having is nginx 5xx and 429 at the edge, which the application's metrics structurally cannot see, and it is worth writing against real data rather than on spec.
...
### Shutdown: every Valkey connection is closed by hand (2026-09-18)

`@toxicoder/nestjs-valkey` registers **no** `onModuleDestroy` — its
`ValkeyService` exposes `disconnect()` and nothing calls it — so `app.close()`
left the socket open and the event loop with it. Both wrappers now close their
own client on destroy: `ValkeyModule` (`~lib/valkey`) the session one and
`CacheModule` (`~lib/cache`) the cache one, each resolving `ValkeyService`
from its own module scope so neither can close the other's.
`ValkeyPubSubService` already did this for the connection it duplicates, and
its comment says why; the primary connections were simply nobody's job.

What it cost while it was missing: **a standalone script never exited.**
`pnpm reconcile-flavors` printed its summary, closed the application and then
hung — two of them sat on the machine for eight hours. The same leak is what a
container pays on `SIGTERM`: the process answers the signal, runs its
shutdown hooks, and then waits out `stop_grace_period: 60s` before being
killed, so every deploy spent a minute per replica doing nothing. Anything
that builds a Nest context around these modules — a script, a test, the API —
now exits when its work is done.
...
| `GET /product/review/summary?includeAcknowledged=` — the counters the screen's tabs and its «Проблеми» dropdown badge themselves with: `open`, `verifiedToday`, `rejected`, `conflictOnly`, `byIssue` (per code, overlapping, so never summed), `knowledgeBaseAppliedAt` and the producers queue's own `{open, byIssue}`. The checkbox's own flag changes the numbers beside it, since an acknowledged contradiction queues nothing until it is asked for                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                            | `product:review`                             |
| `GET /product/review/queue?issue=&store=&name=&status=open\|verified\|rejected&sort=severity\|newest\|reach&includeUnstocked=&includeAcknowledged=&page=&perPage=` — the curation queue, **worst first** by default. `issue` and `store` are both comma-separated lists, and a row survives when it carries any one of the codes and is listed by any one of the shops. A row carries every fact with its source, `producer`/`bottler` as `{id, slug, name, kind, status}`, `producerSource`, `brandOrig`, `matchKey` (nowhere else in the API), the flavour tags, every shop's `offers[]` (with the brand token hidden in each raw name), the `conflicts[]` recorded against it, and `issues[]` — each `{code, severity, field?, detail?, producerId?}`, where `producerId` accompanies the two codes whose `detail` names a producer, so the screen can open the row rather than only quote it. `status` absent is the work; the other two are the decision logs, and `rejected` is readable because a rejection made by mistake is only reversible if the row can still be found                                                                                                                                                                  | `product:review`                             |
| `GET /product/review/:id/suggestions` — everything the side panel loads when a bottling is opened: `producers[]` (candidates with the spelling that found each and **how** — a stated brand, the `ТМ` token, an alias the resolver is forbidden to look for inside a name, a name word), `duplicates[]` (exact twins and near-twins), `siblings` (what identically-named bottlings say about the facts this one lacks) and `storeHints` (words lifted out of the shops' own listing URLs)                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                            | `product:review`                             |
| `POST /product/review/preview` `{productId, producer}` — what a producer action would do before it is taken: `affected[]`, `affectedTotal`, `reach` and, for an alias, the same `{frees, steals}` pair per scope. Read-only; writes nothing and bumps nothing                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                        | `product:review`                             |
| `POST /product/review/commit` `{productId, verdict, patch?, confirm?, producer?}` → `{productId, merged, created, issuesLeft, affected, affectedTotal}` — one request records everything a person decided about one bottling. Every written field is stamped `manual`, `confirm[]` stamps without changing a value, the producer link is a pin, a new alias or a widened one, every open contradiction of the bottling is acknowledged, and the verdict lands on the **merge survivor**. `issuesLeft` is read back through the same detectors the queue uses                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                         | `product:review`                             |
| `POST /product/review/merge` `{sourceId, targetId}` — fold one bottling into another explicitly, for a duplicate with many offers where `relink` (one listing) is the wrong tool. The vanishing row's key is retired into `product_match_alias`, so the next sync cannot recreate it                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                 | `product:review`                             |
| `POST /product/review/bulk` `{productIds, typeName?\|countryCode?\|producerId?}` — write exactly one fact across up to 200 bottlings, every value `manual`, one transaction                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                          | `product:review`                             |
| `POST /product/review/apply` — re-resolve the whole catalogue against the knowledge base and write what it implies, answering `200` with what was written. **This is the other half of every review decision**: promoting a producer stores a claim and changes nothing a filter reads until this runs. Shares `KbReconcileService` verbatim with `pnpm reconcile-flavors`; idempotent, so a second call reports zeros                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                               | `product:review`                             |
| `POST /product/review/status` `{productIds, reviewStatus}` → `200 {updated, products}` — record a verdict on up to 200 bottlings. One route for all three transitions, since they differ only in the value written and un-rejecting _is_ verifying; **bulk** because a pass over a night's arrivals would otherwise earn a 429 from the per-caller limiter                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                           | `product:review`                             |
| `GET /producer?kind=&status=&name=&sort=&order=&page=&perPage=` — the producers section's listing: distilleries, brands, blends and bottlers in one paginated table. **`sort` absent is a state, not a default** — it means the listing's own order (most bottlings first), which is what the column headers' third click returns to; `name` is a case-insensitive substring of the name or the slug. Rows carry `createdAt`, the two link ids and their names beside the slugs                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                      | `producer:read`                              |
| `GET /producer/search?q=&kind=&limit=` — the picker read behind the parent, bottler and link fields: id, slug, name, kind, status and the country's flag. Matches the name, the slug **or** any alias, and unlike `/brand/search` it offers `unverified` rows too — a brand is linked to a distillery nobody has confirmed yet far more often than not. A blank `q` answers the head of the list, so an untouched picker has something to pick                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                       | `producer:read`                              |
| `POST /producer` — create a producer **and apply it**, answering `{producer, applied, skippedAliases}` like the patch does. The name is always added as an alias alongside whatever `aliases` the body lists: a producer nothing resolves to changes nothing. **`rules` creates the producer's name-pattern rules with it** (same body as `POST /producer/:id/rule`, at most 20), in one transaction with the row and its spellings — a rule is scoped to an id that exists only once the row is written, so nothing else could give a create the peat exceptions a maker is defined by. Every rule is validated before the write, so a malformed one is `400` with nothing written; two rules stating the same `(pattern, flavor)` are `409` and the whole create rolls back. Defaults to `status: verified` — a person typed the row, which is what that status means. A taken slug is `409` **carrying the holder** — `{message, data: {producer: {id, slug, name, status, note}}}`, the one JSON error body in this API — because that holder is usually a `rejected` row no picker offers, so without it the name reads as free and the refusal explains nothing; a `parentId`/`bottlerId` naming no producer is `400`, not a foreign-key `500` | `producer:create`                            |
| `POST /producer/:id/alias` `{brand, scope?, note?}` — point one shop spelling at a producer **and apply it**, answering the reconcile summary. An alias rather than a per-bottling link on purpose: the resolver reaches a producer only through `producer_alias`, so one alias fixes every bottling carrying that spelling and the country and type then flow from the knowledge base. `brand` is normalized by `KbKeyUtils.key`, so a raw shop string may be pasted; a spelling another producer already claims is `409` naming it, while re-adding this producer's own **rescopes it** — the `Hyde` case, where the key exists brand-scoped and the person is choosing where it may be matched                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                    | `producer:update`                            |
| `PATCH /producer/:id/alias/:aliasId` `{scope}` — widen or narrow one spelling **and apply the change**. The one-click «→ на початку назви» of the producer modal, and the reason the `lead` scope exists at all                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                      | `producer:update`                            |
| `GET /producer/:id/alias/:aliasId/preview[?scope=]` — what touching one spelling would do, before it is touched. Without `scope`, the removal: the bottlings that would stop resolving to the producer and come back to the queue — the one action here whose reach is entirely destructive, so the one that most needs stating. With `scope`, the rescope: what the spelling would reach once it may be matched there, the number «→ на початку назви» carries on the producer card, answered by the same pass the binding block's preview runs so the two screens cannot number one action differently                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                             | `producer:read`                              |
| `DELETE /producer/:id/alias/:aliasId` — remove one spelling **and apply the removal**. Scoped to the producer, so a stray id cannot reach another maker's spelling                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                   | `producer:update`                            |
| `GET /producer/review?issue=&status=&kind=&name=&page=&perPage=` — the producers queue: makers that need a person, with `issues[]`, `aliasCount`, `unresolvedMentions` and, on the withheld slice, `potentialReach`. Membership mirrors the bottlings queue — `unverified`, or live with something worse than an information code — and `rejected` is never queued, since that verdict is the answer. Codes: `no-alias` (18 today, nothing can ever resolve to it), `alias-unreachable` (8, every spelling is brand-scoped or under the five-character floor while an unresolved bottling carries the word), `unlinked-mentions`, `kind-suspect` (13, a `blend` whose bottlings are mostly single malts), `no-region`, `unverified`, `no-default-type`, `brand-no-parent`, `peat-unknown`                                                                                                                                                                                                                                                                                                                                                                                                                                                            | `producer:read`                              |
| `GET /producer/owner?q=&limit=` — the distinct owning companies matching `q`, as `{name}` rows. Matched `lower(owner) LIKE '%'                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                       |                                              |
...
| `GET /producer/:id` — one producer plus the things that override its facts: its child lines (`parentId`), its own name-pattern rules, the global peat rules as read-only context, and **its aliases** — the spellings that reach it, which are the only way anything does. A peat band cannot be judged without the first three, and a row with no alias resolves nothing at all                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                     | `producer:read`                              |
| `PATCH /producer/:id` — edit a producer **and apply it**: the catalogue is re-resolved in the same request and the response is `{producer, applied}`, where `applied` is what the pass wrote. Storing the decision alone changes nothing a filter reads, so the two are one action; the pass costs ~200 ms, is idempotent and never touches a `manual` value. Nullable fields come in pairs (value + a `clear*` flag) so a reviewer fixing one field cannot silently wipe another — six pairs now, `parentId` and `bottlerId` among them, which is how a brand is linked to the distillery it comes from and a range to the bottler that released it. `status` carries the verdict: `verified` promotes the row, `rejected` rules it out as not a whisky producer, `unverified` puts it back in the queue                                                                                                                                                                                                                                                                                                                                                                                                                                            | `producer:update`                            |
...
Six read-only endpoints under `/dashboard`, all `dashboard:read` (2026-09-15 — they were `Resource.AUTHENTICATED` until then, i.e. open to every signed-in account). Admins bypass the scope as they bypass every other one; a user without it gets a `403` from all six, and the web client hides the screen and its menu entry (`useHasPermission('dashboard:read')`). A grant reaches the caller only on their next token refresh, since scopes are baked into the access token — that is true of every permission, but it is the one thing to check when a fresh grant seems not to have taken.
...
- **The schema ships as a migration; the rates do not.** `currency-rate`
  ...
  `docker compose exec be node dist/scripts/currency-rates.js --full` is
...
- **Purchases are saved with the row, never on their own.** `PATCH
  /collection/:id` takes an optional `purchases` block — `add` (each entry a
  purchase as `POST /collection` accepts it), `update` (each entry a purchase
  id plus the patch fields below) and `remove` (ids) — and applies the row
  patch and all three groups in one transaction, removals first, then patches,
  then additions. The edit screen has a single «Зберегти», so one save is one
  request and either lands whole or not at all. A purchase named in both
  `update` and `remove` is `400`; an id listed twice in `remove` is deleted
  once; a purchase id belonging to another row — even the same user's — is
  `404`. Ownership is settled by loading the row before any write, since a
  purchase-only request carries an empty row patch. The three per-purchase
...
The project builds, `tsc`/`eslint` are clean, and 1285 unit tests (98 suites)
plus 317 integration tests (29 suites, live Postgres and Valkey) pass — bar one
pre-existing data-dependent failure in `push-digest.integration.spec.ts`, which
reads the shared development catalogue instead of owning its rows
([`FOLLOWUPS.md`](FOLLOWUPS.md) item 9). Done:
...
- **Dashboard API is built** (`domain/dashboard`, 2026-08-22): six read-only
  ...
  `movers`, `sync-activity`), all `dashboard:read` (2026-09-15; they were
  `Resource.AUTHENTICATED` until then) + `@CacheControl(600)`, powering the
  web dashboard's time-series charts. The
...
- **NBU currency rates are built** (`core/currency`, `domain/currency`,
  ...
  the source across the units boundary; deployed to prod on 2026-09-06 and
  backfilled there with `dist/scripts/currency-rates.js --full` (the
  procedure is in `docs/CURRENCY-RATES-PROD.md`).
